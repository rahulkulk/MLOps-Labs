...
### CHANGES DONE:
In this updated DAG, I made two key changes. First, I replaced the standard KMeans clustering with MiniBatchKMeans, which processes data in small batches, making it faster and more memory-efficient for larger datasets. Second, I used Principal Component Analysis (PCA) for dimensionality reduction before clustering, which helps reduce noise, speeds up computation, and can improve clustering performance by focusing on the most informative features. 